...
OxEpNamespace.ThemeMaker2 = {
	...
	install: function(callWhenInstalled) {
		...
		/*
		From Matthias:
		We no longer have an instance of less.js in UI, so I load it via CDN:
		(Note inject js directly from chrome ext to avoid cross site error)
		$('head').append($('<script src="http://cdnjs.cloudflare.com/ajax/libs/less.js/1.3.3/less.min.js">'));

		We need to call the parser on-the-fly, might be officially documented somewhere but I didn't manage to find it. 
		This helped: https://github.com/cloudhead/less.js/issues/626
		*/

		//Look at JQueryUI ThemeRoller for examples

		var parser = less.Parser();
		// errors are rather line-based, so \n helps debugging
		var source = [
			'@topbar-color: #c00;',
			'#io-ox-topbar {',
			...
			'}'].join('\n');
		parser.parse(source, function (error, result) {
			if (error) {
				console.error(error);
			} else {
				var css = result.toCSS();
				console.log('css', css);
...
OxEpNamespace.ThemeMaker2.install();